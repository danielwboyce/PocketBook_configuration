...
-- 
-- Utltimately copied from https://gist.github.com/IntrovertedMage/6ea38091292310241ba436f930ee0cb4
...
function ProgressWidget:_setColors(thin)
    local read, unread = colorAttrib(true), colorAttrib(false)
    local readColor = Settings:get(thin, read);
    if not readColor:match("^#%x%x%x%x?%x?%x?$") then
	readColor = "#808080"
	Settings:set(thin, read, readColor)
    end
    
    local unreadColor = Settings:get(thin, unread);
    if not unreadColor:match("^#%x%x%x%x?%x?%x?$") then
	unreadColor = "#ffffff"
    	Settings:set(thin, unread, unreadColor)
    end
